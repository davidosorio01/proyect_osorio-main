<script lang="ts">
    ...
    export let filteredDeportes: Array<{ name: string, logo: string, href: string }>;
    ...
    filteredDeportes = cards;

    function handleCardClick(href: string) {
        if (href === '/empezar') {
            Swal.fire({
                title: '¡Próximamente!',
                text: 'Este deporte estará disponible muy pronto.',
                icon: 'error',
...
<div class="box-info" id="sports-container">
    {#each filteredDeportes as deporte}
        <div class="sport-item" on:click={() => handleCardClick(deporte.href)}>
